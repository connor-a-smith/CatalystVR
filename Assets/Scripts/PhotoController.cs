...
public class PhotoController : MonoBehaviour {
  ...
  [SerializeField] private GameObject photoCenter = null;
  ...
  void Start() {
    ...
    if (photoCenter == null) {
      ...
      photoCenter = Camera.main.gameObject;
  ...
  private IEnumerator LoadPhoto() {
    ...
    for (int row = 0; row < pictureLocations.Count; row++) {
      ...
      for (int col = 0; col < pictureLocations[row].Count; col++) {
        ...
        photos[row][col].SetPhotoInfo(row, col, (360.0f / pictureLocations[row].Count));
  ...
  public IEnumerator Rotate(Vector3 rotationDelta, float duration) {
    ...
    movingPhotos = true;
    Vector3 startRotation = photoHolder.transform.rotation.eulerAngles;
    ...
    for (float time = 0; time < duration; time += Time.deltaTime) {
      ...
      photoHolder.transform.rotation = Quaternion.Euler(Vector3.Lerp(startRotation, endRotation, time / duration));
      yield return null;

    }

    photoHolder.transform.rotation = Quaternion.Euler(endRotation);
